...
<script> 
	  // $('#schedyear option').each(function(){
	  //     var a=$(this).text();
	  //     var len=a.length-2;
	  //     $(this).html(a.substr(1,len))
	  //   });
$(function() {
...
<input type="hidden"  id="schedyear">
...
<!-- <select id="schedyear" style="position:absolute" multiple="" size="4">
	...
	<tr id="listcourses">
		...
		<?php
			...
			while ($row=mysql_fetch_array($course)) {
				?>
					<td class="regcourselist colid<?=$row['course_id'];?>" style="text-align:center"><div style="position:relative"><?=$row['acronym'];?><button class="removecourse" onclick="removecourse(<?=$row['course_id'];?>,'removedummy')"></button></div></td>
				<?php
			}
		?>
 	</tr>
	...
	<tr>
		...
		while ($schoolfeerow=mysql_fetch_array($schoolfee)) {
			...
			<td class="miscdesccon"><div style="position:relative">
				<input type="text" <?php echo "$readonly"; ?> class="miscdesc" onkeyup="checkdescvalue(this)" name="<?=$schoolfeerow['payment_id'];?>" group="misc" value="<?=$schoolfeerow['payment_desc'];?>" refreshval="<?=$schoolfeerow['payment_desc'];?>">
				<div class="dupentry2"><div></div>Duplicate entry</div>
				<span><?=$schoolfeerow['payment_desc'];?></span>

				<button class="removeadd" title="Remove" onclick="removeadd(<?=$schoolfeerow['payment_id'];?>)" style="<?=$hide;?>"></button>
				</div>
			</td>
			<td>
				<?php
...
<table id="gradtable" border>
	...
	<tr>
		<td>Description<button onclick="addgrad()" style="padding:2px 5px 2px 5px">+</button></td><td>Amount</td><td>Payment Category</td>
	</tr>
	<?php
