...
header("Content-Disposition: attachment;Filename='$month/$year Monthly.xls'");
...
@media print(){
.tuicatheader {background:#abdba5;}
.misccatheader {background:#e2d89f;}
.tfcatheader{background: #8d8d7f}
}
	#monthlytable td {padding:2px }
	#monthlytable .amount,.totalbottom {text-align:right;}
	.totalbottom {font-weight:bold;} 
  </style>
 <table id="monthlytable" border="1px solid red" style="font-size:15px;min-width:100%;border-collapse:collapse;position:relative ">
	<?php
	...
	<tr>		
			...
			// echo "$tuithrow[payment_desc] <br>";			
				if($tuithrow['payment_desc']=="Laboratory Fee"){
					...
					echo "<td colspan='$countdept' style='text-align:center'>Laboratory Fees</td>";
						...
						echo "<td rowspan='2'  style='max-width:120px; word-wrap: break-word;'>$tuithrow[payment_desc]</td>";
					}
			}
		...
		while ($miscthrow=mysql_fetch_array($miscth)) {
			...
			echo "<td rowspan='2'  style='max-width:120px; word-wrap: break-word;'>$miscthrow[payment_desc]</td>";
				}
		}
		?>
		...
		while ($tfthrow=mysql_fetch_array($tfth)) {
			echo "<td rowspan='2'  style='max-width:120px; word-wrap: break-word;'>$tfthrow[payment_desc]</td>";
		}
		?>
				 	...
				 	while ($alldeptrow=mysql_fetch_array($alldept)){
						...
						$studdeptrow=mysql_fetch_array($studdept);
  						if($alldeptrow['dept_id']==$studlabamountrow['dept_id']){
							...
							echo "<td $cancel  style='$strike;text-align:right'   class='tuicat tuitd amount coliddept$alldeptrow[dept_id]'>".number_format($studlabamountrow['amount'],2)."</td>";
							
							
						}else{
							echo "<td $ancel   style='$strike;text-align:right'  class='tuicat tuitd amount coliddept$alldeptrow[dept_id]'>0.00</td>";
						}
 			...
 			while ($tfthrow=mysql_fetch_array($tfth)){				
				//get amount of every payment
					...
					if($tfamountrow['amount']==""){
						$totalarray[$startarray]=$totalarray[$startarray]+0;
						echo "<td $cancel style='$strike' class='tfcat amount colid$tfthrow[payment_id]' style='text-align:right'>0</td>";
					}else{
						$totalarray[$startarray]=$totalarray[$startarray]+$tfamountrow['amount'];
						echo "<td $cancel style='$strike' class='tfcat amount colid$tfthrow[payment_id]' style='text-align:right'>".number_format($tfamountrow['amount'],2)."</td>";
					}
		...
		$colspan=8;
			while ($tuithrow=mysql_fetch_array($tuith)) {				
				...
				if($tuithrow['payment_desc']=="Laboratory Fee"){
					...
					while ($studdeptrow=mysql_fetch_array($studdept)) {
						...
						$alltuitotal=$alltuitotal+$totalarray[$startarray];
						echo "<td class='totalbottom tuitotalbotom colid$tuithrow[payment_id]' name='$tuithrow[payment_id]' style='text-align:right;font-weight:bold'>".number_format($totalarray[$startarray],2)."</td>";
				$colspan++;
 				}
			}

			echo "<td class='totalbottom tuitotalbotom' style='text-align:right;font-weight:bold'>".number_format($alltuitotal,2)."</td>";
			...
			while ($miscthrow=mysql_fetch_array($miscth)) {	
					...
					while ($deptstart<$deptaarraycount){
						...
						echo "<td style='text-align:right;font-weight:bold' class='totalbottom tuitotalbotom colid$deptaarray[$deptstart]dept' value='0' name='$deptaarray[$deptstart]dept'>".number_format($totalarray[$startarray],2)."</td>";
						$colspan++;
						$deptstart++;
 					}
				...
				$colspan++;
 				...
 				echo "<td  style='text-align:right;font-weight:bold' class='totalbottom misctotalbottom  colid$miscthrow[payment_id]' name='$miscthrow[payment_id]'>".number_format($totalarray[$startarray],2)."</td>";
  				}
  			}
 			echo "<td class='totalbottom misctotalbottom' style='text-align:center;font-weight:bold'>".number_format($allmisctotal,2)."</td>";
		...
		$alltftotal=0;
			while ($tfthrow=mysql_fetch_array($tfth)) {	
				...
				$alltftotal=$alltftotal+$totalarray[$startarray];
 				echo "<td class='totalbottom tftotalbottom  colid$tfthrow[payment_id]' name='$tfthrow[payment_id]' style='text-align:right;font-weight:bold'>".number_format($totalarray[$startarray],2)."</td>";
  			}
