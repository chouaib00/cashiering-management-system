...
<table id="monthlytable" border="1px solid red" style="min-width:100%;border-collapse:collapse;position:relative ">
	...
	<tr>		
		...
		while ($miscthrow=mysql_fetch_array($miscth)) {
			...
			echo "<td rowspan='2' style='text-align:center'>$miscthrow[payment_desc]</td>";
			}
		}
		?>
					...
					$alldept=mysql_query("select * from schedule_of_fees,course,paymentlist,department,collection where   schedule_of_fees.course_id=course.course_id and schedule_of_fees.payment_id=paymentlist.payment_id and course.dept_id=department.dept_id and paymentlist.payment_desc='Laboratory Fee'and collection.sy='$sy' and collection.semester='$semester' group by department.acronym order by department.dept_id")or die(mysql_error());
				 	while ($alldeptrow=mysql_fetch_array($alldept)){
						...
						//get the student department during he paid the lab fee,if he shifted to another course,still the recent course will followed in putting the lab amount in the department category
							...
							echo "<td $cancel   style='$strike;text-align:right'  class='tuicat tuitd amount coliddept$alldeptrow[dept_id]'><a style='color:white'>`</a>".number_format($studlabamountrow['amount'],2)."</td>";
		...
		$miscth=mysql_query("select * from paymentlist where payment_id in ($miscarray) order by payment_id asc");
			while ($miscthrow=mysql_fetch_array($miscth)){	
				...
				if($miscthrow['payment_desc']=="Laboratory Fee"){
					...
					$alldept=mysql_query("select * from schedule_of_fees,course,paymentlist,department,collection where   schedule_of_fees.course_id=course.course_id and schedule_of_fees.payment_id=paymentlist.payment_id and course.dept_id=department.dept_id and paymentlist.payment_desc='Laboratory Fee'and collection.sy='$sy' and collection.semester='$semester' group by department.acronym order by department.dept_id")or die(mysql_error());
				 	while ($alldeptrow=mysql_fetch_array($alldept)){
						  $studlabamount=mysql_query("select SUM(collection.amount) as amount,course.dept_id from collection,schedule_of_fees,department,course where collection.sched_id=schedule_of_fees.sched_id and schedule_of_fees.course_id=course.course_id and course.dept_id='$alldeptrow[dept_id]' and course.dept_id=department.dept_id and schedule_of_fees.payment_id='$miscthrow[payment_id]' and remark='0' and date='$studpayrow[date]'") or die(mysql_error());
							$studlabamountrow=mysql_fetch_array($studlabamount);
							if($studlabamountrow['amount']==0){
								$studlabamountrow['amount']=0.00;
							}
 				...
 				//get amount of every payment
					$miscamount=mysql_query("select SUM(collection.amount) as amount from collection,schedule_of_fees where schedule_of_fees.sched_id=collection.sched_id and  schedule_of_fees.payment_id='$miscthrow[payment_id]'  and date='$studpayrow[date]'  and remark='0'	and category='misc'") or die(mysql_error());
  		 ...
  		 $tfth=mysql_query("select * from paymentlist where payment_id in ($tfarray) order by payment_id asc");
			...
			//check if refunded
			$refunded=0;
			$getpartialrefund=mysql_query("select  exceeded_money.amount from collection,exceeded_money where collection.receipt_num=exceeded_money.receipt_num and date='$studpayrow[date]' and action='Refunded' group by exceeded_money.receipt_num") or die(mysql_error());
			while($partialrefund=mysql_fetch_array($getpartialrefund)){

				$refunded=$refunded+$partialrefund['amount'];
			}
...
<a href="inventory/printmonthlyreport2.php?month=<?=$month;?>&year=<?=$year;?>" target='jakecorn'><button  class="download" style="float:right;position:absolute;bottom:-40px;right:10px"></button></a>
