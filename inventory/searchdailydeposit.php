...
$datefinal=$datearray[1]."/".$datearray[2]."/".$datearray[0];
 if(count($datearray)==3){
$getdate=mysql_query("select * from collection where date='$datefinal' group by date asc");

}else{
	$month=$datearray[1]."/";
	$year= "/".$datearray[0];
	$getdate=mysql_query("select * from collection where date like '$month%' and date like '%$year' group by date asc");
	...
	<?php
		...
		while ($daterow=mysql_fetch_array($getdate)){
			echo "<tr id='col_id$daterow[col_id]'>";
				...
				echo "<td style='text-align:center' class='dailyaction'><a style='color:blue;' onclick='editdaily($daterow[col_id])'>Update</a>&nbsp;<a style='color:blue;display:none' onclick='saveupdatedailydeposit($daterow[col_id],this)' date='$daterow[date]'>Save</a>&nbsp;<a style='color:blue;display:none' onclick='editdaily($daterow[col_id])'>Cancel</a></td>";
			echo "</tr>";
		}
	?>
	<tr>
