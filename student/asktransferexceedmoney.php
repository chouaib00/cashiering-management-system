...
<form onsubmit="return savetransferexceedmoney()">
...
function savetransferexceedmoney(){
	var id = $('#transto').attr('name');
 	var jake=confirm("Are you sure you want to transfer this amount?");
	if(jake==true){
		$.ajax({
			type:'post',
			url:'student/savetransferexceedmoney.php',
			data:{'from_stud_id':'<?=$stud_id;?>','to_stud_id':id,'semester':'<?=$semester;?>','sy':'<?=$sy;?>','receipt_num':'<?=$receipt_num;?>'},
