...
<div id="unpaidsy" style="margin-left:0px">
	<div class="paymentheader">Balances<?=moneytobepaidpersemester()."-- ".moneypaidpersemester();?> <img src="img/loading.gif" id="studloader" style="display:none;position:relative;top:3px"></div>
	...
	while ($statusrow=mysql_fetch_array($getstatus)) {
		...
		while ($schedrow=mysql_fetch_array($sched)){
			...
			$bal=mysql_query("select collection.amount from collection,schedule_of_fees where collection.remark='0' and schedule_of_fees.sched_id=collection.sched_id and  schedule_of_fees.payment_id='$schedrow[payment_id]' and stud_id='$stud_id' and   schedule_of_fees.sy='$statusrow[sy]' and schedule_of_fees.semester='$statusrow[semester]'");
		...
		//check the rle
		// $sched=mysql_query("select * from schedule_of_fees,paymentlist  where schedule_of_fees.payment_id=paymentlist.payment_id and paymentlist.payment_group='rle' and course_id='$statusrow[course_id]' and (year_level like '$statusrow[year_level]&%' or year_level like '%&$statusrow[year_level]') and sy='$sy' and semester='$semester' ") or die(mysql_error());
		...
		// }
		 ...
		 // echo $countbal."AS FSDF $rlerquire";
			// while ($balrow=mysql_fetch_array($bal)) {
			// 	echo "ASFD<TD>AS FSFSD</TD>";
			// 	$moneypaidpersemester=$moneypaidpersemester+$balrow['amount'];
			// }
		...
		while ($miscrow=mysql_fetch_array($misc)) {
			$moneytobepaidpersemester=$moneytobepaidpersemester+$miscrow['amount'];
			 ...
			 $bal=mysql_query("select collection.amount from collection,schedule_of_fees where remark='0' and  schedule_of_fees.sched_id=collection.sched_id and  schedule_of_fees.payment_id='$miscrow[payment_id]' and stud_id='$stud_id' and   schedule_of_fees.sy='$statusrow[sy]' and schedule_of_fees.semester='$statusrow[semester]'");
			while ($balrow=mysql_fetch_array($bal)){
  				$moneypaidpersemester=$moneypaidpersemester+$balrow['amount'];
			}
		 }
		...
		if(mysql_num_rows($sched)==0){
			//this will check if rle is paid and in the current course rle is not needed he is shifter from amdna/midwidfery to another course which doesnt require rle
		 $sched=mysql_query("select * from collection,schedule_of_fees,paymentlist where paymentlist.payment_id=schedule_of_fees.payment_id and collection.remark='0' and schedule_of_fees.sched_id=collection.sched_id and  paymentlist.payment_group='rle' and stud_id='$stud_id' and   schedule_of_fees.sy='$statusrow[sy]' and schedule_of_fees.semester='$statusrow[semester]'");
			$rlerquire="no";
		}else{
			$rlerquire="yes";
		...
		if($statusrow['status']=="grad"){		 
			...
			while ($schedrow=mysql_fetch_array($sched)) {
				$moneytobepaidpersemester=$moneytobepaidpersemester+$schedrow['amount'];
				$bal=mysql_query("select * from collection where collection.remark='0' and sched_id='$schedrow[sched_id]'  and stud_id='$stud_id'");
	 			while ($balrow=mysql_fetch_array($bal)) {
					$moneypaidpersemester=$moneypaidpersemester+$balrow['amount'];
				}
			}
...
<tr>
	<td>Receipt Number:</td><td><input type="text" id="receipt" required="required" value="<?=$suggestor;?>"></td>
</tr>
...
// $('#paytable input:eq(1 )').keyup(function(event) {
//  	var amount=parseInt($('#cash').val());
//  	$('#amounttopay').val(amount);
// 	var total=0;
// 	$('input[checked=checked][class=other]').each(function(){
...
// 	// }
// 	var cash=$('#cash').val();
// 	var topay=$('#amounttopay').val();

// 	var overall=parseInt($('#baltopay').attr("name"));
// 	 if(cash>0 ||topay>0){
// 	 	console.log("gt");
// 		 if(topay>overall){

// 		 	if(overall>0){
// 		 		console.log("overall gt");
// 		 		$('#change').val(cash-overall);
// 		 	}else{
...
// $('#amounttopay').keyup(function(event) {
// 		var topay=parseInt($('#amounttopay').val());
// 		var cash=$('#cash').val();
//  	 		if(cash<topay){
// 	 			$('#amounttopay').val(cash);
...
// 					othertotal=othertotal+parseInt($(this).attr('name'));
// 		  		});
// 				total=othertotal+overall;

// 				if(total>topay){
// 					console.log("11");
//  			 		 $('#change').val(cash-topay);
// 				}else{
// 					console.log("22");
// 					if(cash>topay){
// 						if(othertotal>0){
// 							console.log("33");
// 							$('#change').val(cash-othertotal);
// 						}else{
// 							console.log("your re");
// 							$('#change').val(cash-topay);
// 						}
